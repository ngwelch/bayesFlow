...
```{r}
flowPost = 
  fread(file=file.path("results", "flowPost.csv") ) %>%
...
countryPopPost = 
  fread(file=file.path("results", "countryPost.csv") ) %>%
...
```{r}
countryPopAgePost = 
  fread(file=file.path("results", "countryAgePost.csv") ) %>%
  merge(unlargest200[,.(country_code, iso)], by="country_code") %>%
  .[,country_code:=NULL] %>%
...
```{r}
flowMarginPost = 
  fread(file=file.path("results", "flowMarginPost.csv") )  %>%
  merge(unlargest200[,.(country_code, iso)], by="country_code") %>%
  .[,country_code:=NULL] %>%
...
globalFlowForecast = 
  fread(file=file.path("results", "globalMigrationPost.csv"))[
...
pdf( file.path( "results", "flow_forecast_plots.pdf") )
...
pdf( file.path( "results", "germany_forecast_plots.pdf") )
...
pdf( file.path( "results", "venezuela_forecast_plots.pdf") )
...
fwrite(flowPostSI, file=file.path( "results", "forecast_quantiles.csv"))
```
