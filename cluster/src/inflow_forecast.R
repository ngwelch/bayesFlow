...
args = commandArgs(TRUE) #args=c(100, 1, 0.1, "~/research/raftery/welchRaftery2022")

#Rscript --no-save $srcPath/runMcmc.R $traceMax $mcmcSS $mcmcThin $mcmcBurnin $eta0 $SLURM_ARRAY_TASK_ID
...
destCodes = unlargest200[iso!=thisOrigin,.(iso, country_code)][order(iso)][,.(j=1:.N, country_code)]
...
kappa.mcmc = 
  ...
  .[,j:=as.numeric(j)] %>%
  merge(destCodes, by="j") %>%
  setcolorder(c("country_code", "j")) %>%
  .[order(j)]

kappa.transpose = 
	kappa.mcmc[,-c("country_code", "j")] %>% 
	t() %>% 
	set_colnames( kappa.mcmc$country_code )
fwrite(kappa.transpose, file=paste0(dir, "/forecast/kappa/", thisOrigCode ,".csv"))
...
psi.transpose = 
	psi.mcmc[,-c("country_code", "j")] %>% 
	t() %>% 
	set_colnames( psi.mcmc$country_code )
fwrite(psi.transpose, file=paste0(dir, "/forecast/psi/", thisOrigCode ,".csv"))
