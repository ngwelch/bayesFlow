...
do.pop.predict.flow <- function(inp, outdir, nr.traj, ages, batch=1, batch.size=10, pred=NULL, countries=NULL, keep.vital.events=FALSE, 
	...
	bound.pop.drop <- migration.settings$bound.pop.drop
	...
	global.objects <- c("nr.traj", "nr.traj.subset", "trajSubsetIndex", 
	                    ...
	                    "mu", "phi", "sigma", "pi.dir", "mig.rogers.castro", "bound.pop.drop")
...
get.flow.1traj <- function(time, itraj, work.env, res.env) {
    ...
    for(cidx in 1:res.env$ncountries) {
        ...
        inpc$bound.pop.drop <- res.env$bound.pop.drop
...
sample.migration.trajectory <- function(inpc, itraj=NULL, time=NULL, pop=NULL, 
									popM=NULL, popF=NULL, warn.template=NULL) {
	...
	bound.pop.drop <- inpc$bound.pop.drop
	
	rho <- mu * (1 - phi) + phi * log( out.mig.rate.prev )
	small.pop = ( pop<1e3 )

	log.out.mig.rate.raw <- get.log.out.rate(rho=rho, sigma=sigma, time=time, origin=origin, bound.pop.drop=bound.pop.drop, small.pop=small.pop, itermax=100)
	out.mig.rate.raw <- exp( log.out.mig.rate.raw )
    ...
    if( any( c(outmigF, outmigM) > outflow.sched.supremum) ){ # adjust age schedules
		...
		while( any( c( outmigF, outmigM ) > outflow.sched.supremum) ) { 
			...
			if( (j>=10 & resampleAttempt<=100) | any(c(is.nan(outmigF), is.nan(outmigM))) ){
				...
				#log.out.mig.rate.raw <- get.log.out.rate(rho=rho, sigma=sigma, time=time, origin=origin, bound.pop.drop=bound.pop.drop, small.pop=small.pop, itermax=100)
				out.mig.rate.raw <- out.mig.rate.raw * ((1-(1/10))**resampleAttempt)
				out.mig.count.raw <- round( out.mig.rate.raw * 5 * pop ) 
				outflow.sched <- c( outflow.sched.f, outflow.sched.m ) / sum( outflow.sched.f + outflow.sched.m )
				outmigF <- out.mig.count.raw * outflow.sched[1:21]
				outmigM <- out.mig.count.raw * outflow.sched[22:42]
...
get.log.out.rate <- function(rho, sigma, time, origin, bound.pop.drop, small.pop=FALSE, itermax=100){

	#log.out.mig.rate <- rnorm(n=1, mean=rho, sd=sigma)
	
	iter <- 0	
	while(iter<itermax){
		#out.mig.rate.prop <- rgamma(n=1, shape=rho*sigma, rate=sigma)
		log.out.mig.rate.prop <- rnorm(n=1, mean=rho, sd=sigma)
		out.mig.rate.prop <- exp( log.out.mig.rate.prop )
		popdrop <- 1 - out.mig.rate.prop/200

		if( bound.pop.drop==FALSE ) break
		if( !is.gcc(origin) & !is.labor(origin) & (popdrop > 0.84) ) break
		if( is.gcc(origin) & (popdrop > 0.58) ) break
		if( is.labor(origin) & (popdrop > 0.97) ) break
		if( small.pop==TRUE ) break
...
#		}
	...
	return(log.out.mig.rate.prop)
}


get.pi.1trajectory <- function(country.code, itraj, dir){
	trajDestCodes = colnames( fread(file=paste0(dir, "/kappa/", country.code, ".csv"), header=TRUE, nrows=1) )
	kappaRow = fread(file=paste0(dir, "/kappa/", country.code, ".csv"), header=FALSE, nrows=1, skip=itraj ) %>% as.numeric()
	psiRow = fread(file=paste0(dir, "/psi/", country.code, ".csv"), header=FALSE, nrows=1, skip=itraj ) %>% as.numeric()
	etaRaw = rnorm(n=199, mean=kappaRow, sd=psiRow)
	etaSample = etaRaw - mean(etaRaw)
	piTraj = exp( etaSample ) / sum( exp(etaSample) )	
	return(list(pi.traj=piTraj, dest.codes=trajDestCodes))
}
